...
int main(int argc, char** argv) {
  ...
  // CUSTOMIZE: set your device_id, device_model_id and custom_context here.
  DeviceConfig* config = converse_config->mutable_device_config();
  config->set_device_id("assistant-sdk-fake-device-id");
  config->set_device_model_id("my-sample-smart-tv-2017-v1");
  ...
  while (stream->Read(&response)) {  // Returns false when no more to read.
    ...
    if (response.has_device_action()) {
      std::cout << "\n\nassistant_sdk device_control_json: \n"
                ...
                << "\n\naction_type: \n"
                << response.device_action().action_type()
                << std::endl;

      // CUSTOMIZE: Consume device_control_request_json here, extract the
      // command and custom_response and do client execution here.
    }
  }
